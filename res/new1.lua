  ...
  for l=1, DCT[1][1] * DCT[1][2] do
    ...
    for l=1, 4 * DCT[1][2] do
      for j=1, 8 * DCT[1][1] do
		...
		buffer.set(j + coordX - 1, l + coordY - 1, getPIXEL(l*2-1, j), getPIXEL(l*2, j), "▄")
      end
    end
  end
...
--local newImage = {}
--local abc, symbol1, bckg1, fg1

--newImage.width, newImage.height = gpu.getResolution()
-- for f = 1, newImage.height do
	-- for i = 1, newImage.width do
		-- abc = ((f-1)*newImage.width+i) * 4 - 3
		-- symbol1, bckg1, fg1 = gpu.get(i, f)
		-- newImage[abc] = bckg1
		-- newImage[abc+1] = fg1
		-- newImage[abc+2] = 0
		-- newImage[abc+3] = symbol1
	-- end
--end

local function takeScreenshot(path)
	gpu.setResolution(mxw, mxh)
	local picture = {WIDTH, HEIGHT / 2, {}, {}, {}, {}}
	local foreground, background, symbol
		
	for j = 1, HEIGHT / 2 do
		for i = 1, WIDTH do
			symbol, foreground, background = gpu.get(i, j)
			table.insert(picture[3], background)
			table.insert(picture[4], foreground)
			table.insert(picture[5], 0x00)
			table.insert(picture[6], symbol)
		end
	end
	
	--picture.width, picture.height = 1, 1
	
	image.save(path, picture, 6)
end

takeScreenshot("/test12345678.pic")

os.sleep(2)

gpu.fill(1, 1, mxw, mxh, ' ')
local picture = image.load("/test12345678.pic")
image.draw(20, 20, picture)
