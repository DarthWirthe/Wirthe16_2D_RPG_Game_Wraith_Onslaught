...
local game = {
	...
	_parameter = {							-- Параметры
		...
		intRegenMultiplier = 0.22,
		survRegenMultiplier = 0.21,
									...
									action=function()
										if unicode.len(game._gui.mainMenu[1].obj[4].txt) >= 5 then
											if game._gui.mainMenu[1].class == 1 then
												game._player.gudId = 1
												game._player.class = 1
											elseif game._gui.mainMenu[1].class == 2 then
												game._player.gudId = 122
												game._player.class = 2
											end
											game._player.name = game._gui.mainMenu[1].obj[4].txt
...
local pmov = 0
local bufferenv = {} -- Список для буфера картинок
...
local playerParams = {
	...
	[2]={
		...
		["intmpmul"]=15,
		["skills"]={
		{1,0,1},{11,0,1},{12,0,1},{13,0,1},{14,0,0},{15,0,0},{17,0,0},{18,0,0},{16,0,0}
		}
	}
}
...
game._function.version = {1,2,8,2}
...
function game._function.initResources()
	...
	local playerSkills = playerParams[game._player.class]["skills"]
	for f = 1, #playerSkills do
		game._player.skills[f] = {}
		for n = 1, #playerSkills[f] do
			game._player.skills[f][n] = playerSkills[f][n]
		end
	end
...
function game._function.random(n1,n2,accuracy)
	...
	return game._function.roundUpNumber(math.random(n1*ass,n2*ass))/ass
end
...
local function setSpritePosition(id, pos)
	--if id ~= game._player.id and CGD[id]["spos"] ~= pos then
		--imageBuffer[CGD[id]["image"]] = image.flipHorizontally(imageBuffer[CGD[id]["image"]])
	--end
	CGD[id]["spos"] = pos
end

local function setPlayerImage(img)
	CGD[game._player.id]["image"] = img
	--setSpritePosition(game._player.id, 1)
end
...
function game._function.roundUpNumber(num)
	local res
...
function game._function.getPlayerAtdsBySkill(skill)
	return ( vAttackDistance or 8 ) + gsd[game._player.skills[game._player.actSkills[skill]][1]]["distance"]
end

local function insertQuests(id, dialog) -- Тест
	local newDialog = dialog
	local qList = gud[CGD[id]["id"]]["quests"]
	local insQuestDialog = true
	local gtext, atext, rtext = "Новое задание", "Я выполню это задание", "Не сейчас"

	if type(qList) == "table" and game._gui.NPCDialog.currentDialog["im"] ~= nil then
		...
		for q = 1, #game._player.quests do
			-- Если задание завершено и отдаётся этому НИПу (["qr"])
			if game._player.quests[q][3] and CGD[CGD[game._player.id]["target"]]["id"] == gqd[game._player.quests[q][1]]["qr"] then
				tableInsert(newDialog[1]["do"], {["text"]=gqd[game._player.quests[q][1]]["name"], ["action"] = "cmpquest", ["do"]=game._player.quests[q][1]})
			end
		end

		for f = 1, #qList do
			if gqd[qList[f]]["comp"] == 0 and gqd[qList[f]]["minlvl"] <= CGD[game._player.id]["lvl"] then -- Только доступные задания
				tableInsert(newDialog[1]["do"], {["q"]=qList[f], ["text"]=gqd[qList[f]]["name"], ["action"] = "qdialog",
				["do"] = {
					["text"]=(gqd[qList[f]]["gtext"] or gtext),
					{["text"]=(gqd[qList[f]]["atext"] or atext), ["action"] = "getquest", ["do"]=qList[f]},
					{["text"]=(gqd[qList[f]]["rtext"] or rtext), ["action"] = "close", ["do"]=nil}
					}
				})
			end
		...
		tableInsert(newDialog[1]["do"], {["text"] = "До встречи", ["action"] = "close", ["do"]=nil})
	end
	return newDialog
end
...
local function setImage(unitID)
	...
	tableInsert(imageBuffer, #imageBuffer + 1, image.load(game._data.text.directory.."sprpic/"..gud[unitID]["image"]..".pic"))
...
function game._function.addUnit(id, x, y)
	...
	CGD[new]["image"] = setImage(id) -- тест
	CGD[new]["width"] = imageBuffer[CGD[new]["image"]][1]
	CGD[new]["height"] = imageBuffer[CGD[new]["image"]][2]
	...
	if (id ~= 1 and gud[CGD[new]["id"]]["rtype"] == 4 or gud[CGD[new]["id"]]["rtype"] == 1) and new ~= game._player.id then
...
local function updateItemQuests()
	-- Задание на предметы
...
function game._function.checkInventoryisFull()
	local full = true
	for f = 1, #game._player.inventory["bag"] do
		if game._player.inventory["bag"][f][2] == 0 then full = false end
	end
	return full
end
...
function game._function.checkInventorySpace()
	local space = 0
	for f = 1, #game._player.inventory["bag"] do
		if game._player.inventory["bag"][f][1] == 0 then space = space + 1 end
	end
	return space
end
...
function game._function.addItem(itemid, num, indicate)
	...
	for f = 1, #game._player.inventory["bag"] do
		if game._player.inventory["bag"][f][2] == 0 then
			game._player.inventory["bag"][f][1] = 0
			if game._player.inventory["bag"][f][1] >= game._config.reservedItemId then -- Возможна Ошибка
				gid[game._player.inventory["bag"][f][1]] = nil
			end
	...
	if gid[itemid] and not gid[itemid]["stack"] then
		for f = 1, #game._player.inventory["bag"] do
			if game._player.inventory["bag"][f][1] == 0 then
				itemAdded = 1
				game._player.inventory["bag"][f][1] = itemid
				game._player.inventory["bag"][f][2] = num
				if game._data.windowThread == "inventory" then
					iconImageBuffer[f] = image.load(game._data.text.directory.."itempic/"..game._data.itemIcons[gid[game._player.inventory["bag"][f][1]]["icon"]]..".pic")
				end
				position = f
				break
			end
	...
	-- Добавить стакающийся предмет
		...
		for i = 1, #game._player.inventory["bag"] do
			-- Сгруппировать одинаковые
			if game._player.inventory["bag"][i][1] == itemid then
				game._player.inventory["bag"][i][2] = game._player.inventory["bag"][i][2] + num
				itemAdded = 1
				position = i
				break
			end
		...
		if itemAdded == 0 then
		for i = 1, #game._player.inventory["bag"] do
			if game._player.inventory["bag"][i][1] == 0 or game._player.inventory["bag"][i][2] == 0 then
				game._player.inventory["bag"][i][1] = itemid
				game._player.inventory["bag"][i][2] = num
					itemAdded = 1
					if game._data.windowThread == "inventory" then
						iconImageBuffer[i] = image.load(game._data.text.directory.."itempic/"..game._data.itemIcons[gid[game._player.inventory["bag"][i][1]]["icon"]]..".pic")
					end
					position = i
					break
				end
	...
	-- Переполнение
	if itemAdded == 0 and game._function.checkInventoryisFull() then
		lostItem = {itemid,num}
		...
		updateItemQuests() -- тест
		game._gui.qPanel.update() -- тест
	end
	-- Нестакающиеся предметы
	for f = 1, #game._player.inventory["bag"] do
		if game._player.inventory["bag"][f][1] ~= 0 and gid[game._player.inventory["bag"][f][1]] and not gid[game._player.inventory["bag"][f][1]]["stack"] and game._player.inventory["bag"][f][2] > 1 then
			game._player.inventory["bag"][f][2] = 1
		end
	end
	return position
end
...
local function removeItemFromInventory(invid, count)
	count = count or 1
	if game._player.inventory["bag"][invid][2] > 1 then
		game._player.inventory["bag"][invid][2] = game._player.inventory["bag"][invid][2] - count
	else
		game._player.inventory["bag"][invid] = {0, 0, {}}
	end
	updateItemQuests() -- тест
end

-- Забрать предметы из разных ячеек. Вернёт 'true' если нет такого количества.
function game._function.removeItem(iid, count) 
	...
	for f = 1, #game._player.inventory["bag"] do
		if game._player.inventory["bag"][f][1] == iid then
			num = num + game._player.inventory["bag"][f][2]
		end
	end
	...
	if num >= count then
		for f = 1, #game._player.inventory["bag"] do
			if game._player.inventory["bag"][f][1] == iid then
				if game._player.inventory["bag"][f][2] < i then
					removeItemFromInventory(f, game._player.inventory["bag"][f][2])
					i = i - game._player.inventory["bag"][f][2]
				else
...
local function createNewItem(itemID)
	...
	if gid[itemID]["type"] == 2 or gid[itemID]["type"] == 3 then
		...
		local itemAddProperties = {
			[1]={"hp+",
				["min"] = 4 + level ^ 2,
				["max"] = 5 + level ^ 2 * 3,
				[3] = 10 + level * 2, -- % для оружия
				[2] = 80 + level * 2 -- % для брони
				},
			[2]={"sur+",
			...
			[6]={"pdm+",	-- Физ. атака, оружие только тип 1, 2, 3
		...
		while #newDds < mathMin(adnum,gid[itemID]["lvl"]) do
			addThisProp = false
			rand = game._function.random(1, #itemAddProperties)
			if game._function.random(1, 10^5) <= itemAddProperties[rand][gid[itemID]["type"]] * 10^3 then
				value = mathFloor(game._function.random(itemAddProperties[rand]["min"] * 10, itemAddProperties[rand]["max"] * 10) / 10)
				if itemAddProperties[rand]["sub"] then
					for j = 1, #itemAddProperties[rand]["sub"] do
						if gid[itemID]["subtype"] == itemAddProperties[rand]["sub"][j] then
							addThisProp = true
						end
					end
				elseif value >= 1 then
				...
				if addThisProp then
					tableInsert(newDds, {itemAddProperties[rand][1], mathFloor(value)})
				end
			end
		end
...
function game._function.checkItemInBag(itemid)
	local d = 0
	for f = 1, #game._player.inventory["bag"] do
		if game._player.inventory["bag"][f][1] == itemid then
			d = d + game._player.inventory["bag"][f][2]
		end
	end
	return d, itemid
end
...
function game._function.UpdatePlayerStats()
	game._player.updStats = game._function.setAllValuesInArrayTo(game._player.updStats, 0)
	local temp, critRate
	-- Суммирование защиты от снаряжения --
	for k, _ in pairs(game._player.inventory["weared"]) do
		if game._player.inventory["weared"][k][1] ~= 0 and gid[game._player.inventory["weared"][k][1]]["props"]["dds"] then
		temp = gid[game._player.inventory["weared"][k][1]]["props"]["dds"]
			for e = 1, #temp do
				game._player.updStats[temp[e][1]] = game._player.updStats[temp[e][1]] + temp[e][2]
			end
		end
		if game._player.inventory["weared"][k][1] ~= 0 and gid[game._player.inventory["weared"][k][1]]["type"] == 2 then
			game._player.updStats["pdf+"] = game._player.updStats["pdf+"] + gid[game._player.inventory["weared"][k][1]]["props"]["pdef"]
			game._player.updStats["mdf+"] = game._player.updStats["mdf+"] + gid[game._player.inventory["weared"][k][1]]["props"]["mdef"]
		end
	end
    -- Суммирование пассивных умений
	local usingWeapon = false
	local skillLevel
	for f = 1, #game._player.skills do
		skillLevel = game._player.skills[f][3]
		if gsd[game._player.skills[f][1]]["type"] == 2 and gsd[game._player.skills[f][1]]["action"].weaponreq then
			for w = 1, #gsd[game._player.skills[f][1]]["action"].weaponreq do
				if game._player.inventory["weared"]["weapon"][1] ~= 0 and
							gid[game._player.inventory["weared"]["weapon"][1]]["subtype"] == gsd[game._player.skills[f][1]]["action"].weaponreq then
					usingWeapon = true
				end
			end
		else
		...
		if skillLevel > 0 and gsd[game._player.skills[f][1]]["type"] == 2 and game._player.updStats[gsd[game._player.skills[f][1]]["action"].etype] ~= nil
					and usingWeapon == true then
			game._player.updStats[gsd[game._player.skills[f][1]]["action"].etype] = game._player.updStats[gsd[game._player.skills[f][1]]["action"].etype] + gsd[game._player.skills[f][1]]["value"][skillLevel]
		end
	end
 ----
	critRate = 1 + mathFloor((CGD[game._player.id]["strg"]+game._player.updStats["str+"]) / 10)
	critRate = critRate + mathFloor((CGD[game._player.id]["int"]+game._player.updStats["int+"]) / 10)
	critRate = critRate + mathFloor((CGD[game._player.id]["agi"]+game._player.updStats["agi+"]) / 5)

	game._player.updStats["vPdm1"] = game._player.updStats["vPdm1"] + game._player.updStats["pdm+"]
	game._player.updStats["vPdm2"] = game._player.updStats["vPdm2"] + game._player.updStats["pdm+"]
	game._player.updStats["vMdm1"] = game._player.updStats["vMdm1"] + game._player.updStats["mdm+"]
	game._player.updStats["vMdm2"] = game._player.updStats["vMdm2"] + game._player.updStats["mdm+"]

	if game._player.inventory["weared"]["weapon"][1] > 0 then
		if gid[game._player.inventory["weared"]["weapon"][1]]["props"]["phisat"] then
			game._player.updStats["vPdm1"] = game._player.updStats["vPdm1"] + gid[game._player.inventory["weared"]["weapon"][1]]["props"]["phisat"][1]*(1+game._player.updStats["pwd%"]*0.01)
			game._player.updStats["vPdm2"] = game._player.updStats["vPdm2"] + gid[game._player.inventory["weared"]["weapon"][1]]["props"]["phisat"][2]*(1+game._player.updStats["pwd%"]*0.01)
		end
		if gid[game._player.inventory["weared"]["weapon"][1]]["props"]["magat"] then
			game._player.updStats["vMdm1"] = game._player.updStats["vMdm1"] + gid[game._player.inventory["weared"]["weapon"][1]]["props"]["magat"][1]*(1+game._player.updStats["mwd%"]*0.01)
			game._player.updStats["vMdm2"] = game._player.updStats["vMdm2"] + gid[game._player.inventory["weared"]["weapon"][1]]["props"]["magat"][2]*(1+game._player.updStats["mwd%"]*0.01)
		end
		game._player.updStats["weaponDistance"] = game._function.watds[gid[game._player.inventory["weared"]["weapon"][1]]["subtype"]]
		gsd[1]["reloading"] = weaponHitRate[gid[game._player.inventory["weared"]["weapon"][1]]["subtype"]] or 1
	end

	game._player.statsPoints.vSur = game._player.updStats["sur+"]
	game._player.statsPoints.vStr = game._player.updStats["str+"]
	game._player.statsPoints.vInt = game._player.updStats["int+"]
	game._player.statsPoints.vAgi = game._player.updStats["agi+"]
	game._player.statsPoints.vPdm1 = game._player.updStats["vPdm2"]
	game._player.statsPoints.vMdm1 = game._player.updStats["vMdm1"]
	game._player.statsPoints.vPdm2 =game._player.updStats["vPdm2"]
	game._player.statsPoints.vMdm2 = game._player.updStats["vMdm2"]

	CGD[game._player.id]["mhp"] = mathCeil(((playerParams[game._player.class]["basehp"]+(CGD[game._player.id]["surv"]+game._player.updStats["sur+"])*playerParams[game._player.class]["surhpmul"]+(CGD[game._player.id]["lvl"]-1)*playerParams[game._player.class]["levelhpmul"]+game._player.updStats["hp+"]))*(1+game._player.updStats["hp%"]/100))
	CGD[game._player.id]["mmp"] = mathCeil(((playerParams[game._player.class]["basemp"]+(CGD[game._player.id]["int"]+game._player.updStats["int+"])*playerParams[game._player.class]["intmpmul"]+(CGD[game._player.id]["lvl"]-1)*playerParams[game._player.class]["levelmpmul"]+game._player.updStats["mp+"]))*(1+game._player.updStats["mp%"]/100))

	local pAttackBase = 1+(4*(CGD[game._player.id]["strg"]+game._player.updStats["str+"])+(CGD[game._player.id]["agi"]+game._player.updStats["agi+"]))/100
	local mAttackBase = 1+(4*(CGD[game._player.id]["int"]+game._player.updStats["int+"])+(CGD[game._player.id]["agi"]+game._player.updStats["agi+"]))/100
	CGD[game._player.id]["ptk"] = {
		mathFloor(1+pAttackBase*(CGD[game._player.id]["lvl"]+game._player.updStats["vPdm1"])),
		mathCeil(1+pAttackBase*(CGD[game._player.id]["lvl"]+game._player.updStats["vPdm2"]))
	}
	CGD[game._player.id]["mtk"] = {
		mathFloor(1+mAttackBase*(CGD[game._player.id]["lvl"]+game._player.updStats["vMdm1"])),
		mathCeil(1+mAttackBase*(CGD[game._player.id]["lvl"]+game._player.updStats["vMdm2"]))
	}
	CGD[game._player.id]["pdef"] = mathFloor(30+((CGD[game._player.id]["surv"]+game._player.updStats["sur+"])/2+(CGD[game._player.id]["strg"]+game._player.updStats["str+"])/4)*(CGD[game._player.id]["lvl"]+game._player.updStats["pdf+"]/2))
	CGD[game._player.id]["armorpdef"] = game._player.updStats["pdf+"]
	CGD[game._player.id]["mdef"] = mathFloor(30+((CGD[game._player.id]["surv"]+game._player.updStats["sur+"])/2+(CGD[game._player.id]["int"]+game._player.updStats["int+"])/4)*(CGD[game._player.id]["lvl"]+game._player.updStats["mdf+"]/2))
	CGD[game._player.id]["armormdef"] = game._player.updStats["mdf+"]
	CGD[game._player.id]["cmove"] = true
	CGD[game._player.id]["ctck"] = true
	game._player.critChance = game._player.updStats["chc+"] + critRate
	vAttackDistance = game._player.updStats["weaponDistance"]

	game._player.manaReg = mathMax(0, 0.75 + (CGD[game._player.id]["lvl"] - 1) * game._parameter.intRegenMultiplier + CGD[game._player.id]["int"] * 0.1) * game._player.regenMultiplier
	game._player.healthReg = mathMax(0, 0.75 + (CGD[game._player.id]["lvl"] - 1) * game._parameter.survRegenMultiplier + CGD[game._player.id]["surv"] * 0.1) * game._player.regenMultiplier
end
...
function game._function.addCoins(value)
	game._player.coins = mathMax(game._player.coins + value, 0)
	if game._data.windowThread == nil and value ~= nil and value > 0 then
		game._function.textmsg4("Монеты +"..value.."("..game._player.coins..")")
	end
end
...
local function countSavedUnits()
	local count = 0
	for f = 1, #savedUnits do
		count = count + #savedUnits[f]
	end
	return count
end
...
local function saveUnitData()
	local function isAvailable(id)
		local saveUnit = true
		if CGD[id]["id"] == game._player.id or CGD[id]["id"] == game._data.potralUnitId then -- Порталы не сохраняем
			saveUnit = false
		end
		if CGD[game._player.id]["followers"] then -- Союзников не сохраняем
	...
	for f = 1, #CGD do
		if CGD[f] and isAvailable(f) and CGD[f]["living"] ~= true then
			tableInsert(savedUnits[world.current],{CGD[f]["id"],game._function.roundUpNumber(CGD[f]["x"]),game._function.roundUpNumber(CGD[f]["y"]),CGD[f]["resptime"],f})
		end
	end
end
...
function game._function.loadWorld(id,unitList)
	...
	-- Картиночки
	imageBuffer = {
		[-4]=image.load(game._data.text.directory.."sprpic/"..gud[game._player.gudId]["aimage"][4]),
		[-3]=image.load(game._data.text.directory.."sprpic/"..gud[game._player.gudId]["aimage"][3]),
		[-2]=image.load(game._data.text.directory.."sprpic/"..gud[game._player.gudId]["aimage"][2]),
		[-1]=image.load(game._data.text.directory.."sprpic/"..gud[game._player.gudId]["aimage"][1]),
		[0]=image.load(game._data.text.directory.."sprpic/"..gud[game._player.gudId]["image"]..".pic")
	}
	...
	for f = 1, #spawningUnits do
		...
		if gud[spawningUnits[f][1]]["nres"] ~= false then
			if spawningUnits[f][4] == nil then
				...
				-- Проверить, что если этот юнит дохельный
				for n = 1, #savedUnits[id] do
					if savedUnits[id][n][5] == #CGD then -- savedUnits[id][n] = {id, x, y, t, n}
						if savedUnits[id][n][4] > 0 then
							CGD[#CGD]["living"] = false
							CGD[#CGD]["resptime"] = savedUnits[id][n][4]
						end
					end
				end
				-- --
...
-- Телепорт на другие координаты, или на другую карту
...
function game._function.saveGame(savePath,filename)
	...
	for f = 1, 600 do
		if gid[game._config.reservedItemId - 1 + f] then tableInsert(tempTable, gid[game._config.reservedItemId - 1 + f]) end
	end
	...
	f:write(gud[CGD[game._player.id]["id"]]["name"] .. "\n") -- [1] Название
	f:write(ser.serialize(CGD[game._player.id]),"\n") -- [2] Данные игрока
	f:write(ser.serialize(game._player),"\n") -- [3] Ещё данные игрока
	f:write(ser.serialize({world.current}),"\n") -- [4] Разные переменные: 1-текущая карта
	f:write(ser.serialize(tempTable),"\n") -- [5] Предметы из gid
	f:write(ser.serialize(CGD),"\n") -- [6] CGD
	
	tempTable = {}
	for i = 1, #gqd do
		if gqd[i]["comp"] > 0 then
			tableInsert(tempTable,{i,gqd[i]["comp"]})
		end
	end
	f:write(ser.serialize(tempTable),"\n") -- [7] Выполненные/заблокированные задания
	tempTable = savedUnits
	f:write(ser.serialize(tempTable),"\n") -- [8] Чек побитых монстров
	
	tempTable = {}
	for i = 1, #gud do
		if gud[i]["nres"] == false then
			tableInsert(tempTable,i)
		end
	end
	f:write(ser.serialize(tempTable)) -- [9] Чек нересп. монстров
	f:close()
end
...
function game._function.loadGame(savePath, filename)
	...
	if fs.exists(savePath.."/"..filename) then
		...
		for k, _ in pairs(imageBuffer) do
			game._function.unloadImage(imageBuffer[k])
		end
		imageBuffer = {}
		game._function.cleanIconImageBuffer()
		for i = 1, #gqd do
			gqd[i]["comp"] = 0
		end

		for i = 1, 600 do -- тест (??)
			if gid[game._config.reservedItemId + i] then
				gid[game._config.reservedItemId + i] = nil
			end
		end

		-- --

		tempTable = readFromFile(savePath.."/"..filename)

		game._player = ser.unserialize(tempTable[3]) -- Данные игрока

		tempData = ser.unserialize(tempTable[4]) -- Разные переменные
		world.current = tempData[1] -- Можно продолжить список
		-- ... = tempData[2]

		--CGD = {}
		CGD = ser.unserialize(tempTable[6]) -- CGD -- тест

		gud[CGD[game._player.id]["id"]]["name"] = tempTable[1]

		CGD[game._player.id] = ser.unserialize(tempTable[2]) -- Данные игрока
		setPlayerImage(0)
		local spawnX = CGD[game._player.id]["x"]

		tempData = ser.unserialize(tempTable[5]) -- Предметы из gid
		for f = 1, #tempData do -- тест (??)
			gid[tempData[f]["id"]] = tempData[f]
			if gid[#gid]["oid"] then
				gid[#gid]["name"] = gid[gid[#gid]["oid"]]["name"]
				if gid[#gid]["props"] and type(gid[#gid]["props"]) == "table" and gid[#gid]["props"]["dds"] then
					for o = 1, #gid[#gid]["props"]["dds"] do
						gid[#gid]["name"] = "♦"..gid[#gid]["name"]
					end
				end
		...
		tempData = ser.unserialize(tempTable[7]) -- Выполненные/заблокированные задания
		for f = 1, #tempData do
			gqd[tempData[f][1]]["comp"] = tempData[f][2]
		end

		tempData = ser.unserialize(tempTable[9]) -- Чек нересп. монстров
		for f = 1, #tempData do
			gud[tempData[f]]["nres"] = false
		end

		for f = 1, #world do
			savedUnits[f] = {}
		end
		tempData = ser.unserialize(tempTable[8]) -- Чек побитых монстров
		for n = 1, #tempData do
			savedUnits[n] = tempData[n]
		end

		game._function.loadWorld(world.current)
		--CGD[game._player.id]["followers"] = {} -- тест (доделать)

		game._function.teleport(spawnX)
		targetQuest = 0
	end
end
...
function game._function.pbar(x, y, size, percent, color1, color2, text, textcolor, style)
	...
	elseif style == 1 then
		...
		if dw < size / 4 or dw > size * 0.75 then
			buffer.drawText(x + dw - 1, y, color1, "◤")
		end
	end
end
...
function game._function.drawAllCGDUnits()
	local ccl, cx, cy, dx, dy, textName, vpercentr, halfWidth, subtextninfo
	local hpbarwidth, textwidth = 8, 24
	for f = 1, #CGD do
		if f ~= game._player.id and CGD[f] and CGD[f]["living"] and game._function.getDistanceToId(game._player.id,f) <= game._config.unitDrawingDistance then
			halfWidth = CGD[f]["width"] / 2
			cx, cy = mathFloor(CGD[f]["x"]), mathFloor(CGD[f]["y"])
			dx,dy = cx + 75 - cGlobalx, 49 - cy - CGD[f]["height"]
			if CGD[f]["image"] ~= nil then
				if CGD[f]["spos"] == 2 then
					buffer.drawImage(dx, dy, imageBuffer[CGD[f]["image"]], true)
				else
					buffer.drawImage(dx, dy, image.flipHorizontally(imageBuffer[CGD[f]["image"]]), true)
				end
			else
				buffer.drawText(dx, dy, 0xcc2222, "ERROR")
			end
			-- полоска хп над головой
			if f == CGD[game._player.id]["target"] and CGD[f]["rtype"] == 4 then
				...
				textName = tostring(gud[CGD[CGD[game._player.id]["target"]]["id"]]["name"])
				if unicode.len(textName) >= textwidth then
					textName = unicode.sub(textName, 1, textwidth).."…"
				end
				buffer.drawText(mathFloor(dx + (halfWidth - textwidth / 2) + ((textwidth / 2) - (unicode.len(textName) / 2))), dy - 3, 0xffffff, textName)
			-- прогресс в выкапывании
			elseif game._player.pickingUp and game._player.pckTarget == f and CGD[f]["rtype"] == 5 then
				vpercentr = mathCeil((game._player.maxPckTime-game._player.pckTime) * 100 / game._player.maxPckTime) -- Процент
				game._function.pbar(dx + mathFloor((halfWidth - hpbarwidth / 2)),dy - 2,hpbarwidth, vpercentr, 0x009945, 0x444444, vpercentr.."% ", 0xffffff)
			-- галочка над НПС
			elseif CGD[f]["rtype"] == 2 then
...
game._gui.pauseMenu.buttonFunction = {
	...
	[2]=function()
		game._gui.inventory.open()
	end,
	[3]=function()
		game._data.windowThread = "skillsWindow"
		game._gui.playerSkills.updateDescription() -- Обновить текст с ифнормацией
	end,
	[4]=function()
...
function game._gui.playerInfoPanel.draw()
	...
	roundPrc3 = game._function.roundUpNumber(roundPrc3 * 10)
...
function game._gui.targetInfoPanel.draw()
	...
	if roleType == 4 or roleType == 1 or roleType == 6 then
...
game._gui.targetInfoPanel.action["touch"] = function(ev)
	...
	if game._data.windowThread == nil and ev[5] == 0 and CGD[game._player.id]["target"] then
		if clicked(ev[3],ev[4],game._gui.targetInfoPanel.x,game._gui.targetInfoPanel.y+game._gui.targetInfoPanel.h-1,game._gui.targetInfoPanel.x+game._gui.targetInfoPanel.w-1,game._gui.targetInfoPanel.y+game._gui.targetInfoPanel.h-1) then
			if gud[CGD[CGD[game._player.id]["target"]]["id"]]["rtype"] ~= 5 and gud[CGD[CGD[game._player.id]["target"]]["id"]]["rtype"] ~= 2 then
...
function game._gui.skillsTopPanel.draw()
	...
	for f = 1, #game._gui.skillsTopPanel.t do
		...
		if game._player.actSkills[f + 1] > 0 then
			buffer.drawText(x + 3 + (f * 5 - 5), y + 2, 0xffffff, tostring(mathCeil(game._player.skills[game._player.actSkills[f+1]][2] / 10)))
		end
	end
...
function game._gui.NPCDialog.start(NPC)
	...
	for dnum in game._function.dialogsdata:lines() do
		if game._function.gddnum == gud[CGD[NPC]["id"]]["dialog"] then
...
function game._gui.NPCDialog.draw(x, y)
	...
	for f = #game._gui.NPCDialog.currentDialog, 1, -1 do
	...
	for f = 1, #game._gui.NPCDialog.currentDialog do
		if ev[5] == 0 and clicked(ev[3], ev[4], x + 2, y + 14 + f, x + w - 2, y + 14 + f) then
			...
			elseif game._gui.NPCDialog.currentDialog[f]["action"] == "trade" then
				game._function.tradew.loaded = loadfile(game._data.text.directory.."data/trading.data")(game._gui.NPCDialog.currentDialog[f]["do"])
			...
			elseif game._gui.NPCDialog.currentDialog[f]["action"] == "craft" then
				game._function.craftw.loaded = loadfile(game._data.text.directory.."data/crafting.data")(game._gui.NPCDialog.currentDialog[f]["do"])
			...
			elseif game._gui.NPCDialog.currentDialog[f]["action"] == "cmpquest" then
				for t = 1, #game._player.quests do
					if game._player.quests[t][1] == game._gui.NPCDialog.currentDialog[f]["do"] and game._player.quests[t][3] then
						if gqd[game._gui.NPCDialog.currentDialog[f]["do"]]["type"] == 2 then -- Задание на поиск
							for l = 1, #gqd[game._gui.NPCDialog.currentDialog[f]["do"]]["targ"] do
								-- if gid[gqd[game._gui.NPCDialog.currentDialog[f]["do"]]["targ"][l][1]]["subtype"] == 2 then -- "subtype" = 2 - Для задания
									-- for k = 1, #game._player.inventory["bag"] do
										-- if game._player.inventory["bag"][k][1] == gqd[game._gui.NPCDialog.currentDialog[f]["do"]]["targ"][l][1] then
											-- game._player.inventory["bag"][k] = {0,0}
										-- end
									-- end
								-- else
								game._function.removeItem(gqd[game._gui.NPCDialog.currentDialog[f]["do"]]["targ"][l][1], gqd[game._gui.NPCDialog.currentDialog[f]["do"]]["targ"][l][2])
								--end
							end
						end
						-- *награда за задание*
						...
						if qcomp then
							if not gqd[game._gui.NPCDialog.currentDialog[f]["do"]]["afterCompleted"] then
								...
								game._gui.NPCDialog.currentDialog = nil
								break
							elseif gqd[game._gui.NPCDialog.currentDialog[f]["do"]]["afterCompleted"] then
...
function game._gui.rewardChoice.draw()
	...
	if game._gui.rewardChoice.buffer.targ then
		game._function.drawItemDescription(x + game._gui.rewardChoice.w, y)
...
game._gui.rewardChoice.action["touch"] = function(ev)
	for f = 1, #game._gui.rewardChoice.buffer.items do
		if ev[5] == 0 and clicked(ev[3], ev[4], game._gui.rewardChoice.x+4+f*23-23, game._gui.rewardChoice.y + 2, game._gui.rewardChoice.x+4+f*23-2, game._gui.rewardChoice.y + 11) then
			...
			game._function.getItemInfo(game._gui.rewardChoice.buffer.items[game._gui.rewardChoice.buffer.targ][1], game._gui.rewardChoice.buffer.items[game._gui.rewardChoice.buffer.targ][2])
			break
		end
	end
...
function game._function.getItemInfo(id, count)
	...
	local function giiwcAdd(t,c)
		tableInsert(game._data.itemInfo,{tostring(t),c})
	end
	...
	if gid[id] then
		...
		if itemType == 1 then -- Предметы
			if itemsubtype == 1 then
				giiwcAdd("Уровень материала "..tostring(gid[id]["lvl"]), 0xffffff)
			elseif itemsubtype == 2 then
				giiwcAdd("Предмет для задания. Нельзя выбросить иили продать.", 0xffffff)
			end
		elseif itemType == 2 then -- Броня
		...
		elseif itemType == 5 then -- Коробка
			giiwcAdd("Нажмите ПКМ чтобы открыть.", 0xffffff)
		elseif itemType == 7 then -- Амулет хп
			giiwcAdd("Восстанавливает "..tostring(gid[id]["props"]["ics"]).."% здоровья, если оно ниже "..tostring(gid[id]["props"]["r"]).."%", 0xEFEFEF)
		elseif itemType == 8 then -- Яйцо
			giiwcAdd("Нажмите ПКМ чтобы добавить в союзники", 0xffffff)
		elseif itemType == 9 then -- Источник опыта
			giiwcAdd("Нажмите ПКМ чтобы получить "..tostring(gid[id]["props"][1]).." ед. опыта.", 0xffffff)
		end
...
function game._function.drawItemDescription(x,y)
	local source = game._data.itemInfo
	if not source then
...
function game._function.unloadImage(imgObj) -- Тест
	imgObj[3] = nil
	imgObj[4] = nil
	imgObj[5] = nil
	imgObj[6] = nil
...
function game._function.cleanIconImageBuffer()
	for k, _ in pairs(iconImageBuffer) do
		game._function.unloadImage(iconImageBuffer[k])
	end
	iconImageBuffer = {}
end

game._gui.inventory = {x=1, y=1, w=160, h=50, ax = 117, ay = 3, b1={x=2,y=47,w=14}, target = 0, action={}}

function game._gui.inventory.open()
	...
	for f = 1, #game._player.inventory["bag"] do
		if game._player.inventory["bag"][f][1] ~= 0 and game._player.inventory["bag"][f][2] ~= 0 and gid[game._player.inventory["bag"][f][1]] and game._function.mCheck() then
			iconImageBuffer[f] = image.load(game._data.text.directory.."itempic/"..game._data.itemIcons[gid[game._player.inventory["bag"][f][1]]["icon"]]..".pic")
		end
	end

	for f = 1, #game._data.armorType do
		if game._player.inventory["weared"][game._data.armorType[f]][1] ~= 0 and game._function.mCheck() then
			iconImageBuffer[game._data.armorType[f]] = image.load(game._data.text.directory.."itempic/"..game._data.itemIcons[gid[game._player.inventory["weared"][game._data.armorType[f]][1]]["icon"]]..".pic")
		end
	end
end

function game._gui.inventory.close()
	
end

function game._gui.inventory.draw()
	local x, y, w, h = game._gui.inventory.x, game._gui.inventory.y, game._gui.inventory.w, game._gui.inventory.h
	local ax, ay = game._gui.inventory.ax, game._gui.inventory.ay
	...
	buffer.drawText(x+1,y,0xC4C420,"●•. Монеты: "..tostring(game._player.coins))
	...
	for y1 = 1, 4 do
		for x1 = 1, 5 do
			...
			if game._player.inventory["bag"][formula][1] ~= 0 and game._player.inventory["bag"][formula][2] ~= 0 then
				if iconImageBuffer[formula] then
					if gid[game._player.inventory["bag"][formula][1]] then
						buffer.drawImage(xps, yps, iconImageBuffer[formula])
						if game._player.inventory["bag"][formula][2] > 1 then -- Надпись количество
							buffer.drawRectangle(xps, yps+9, #tostring(game._player.inventory["bag"][formula][2]), 1, 0x4A4A4A, 0, " ")
							buffer.drawText(xps,yps+9,0xffffff,tostring(game._player.inventory["bag"][formula][2]))
						end
					else
	...
	for y1 = 1, 4 do
		for x1 = 1, 2 do
			...
			itemType = game._data.armorType[formula]
			if game._player.inventory["weared"][itemType][1] ~= 0 then
				if iconImageBuffer[itemType] then
					if gid[game._player.inventory["weared"][itemType][1]] then
						buffer.drawImage(xps, yps, iconImageBuffer[itemType])
					else
						buffer.drawImage(xps, yps, image.load(game._data.text.directory.."image/itemnotex.pic"))
					end
				else
				...
				if iconImageBuffer[0][itemType] then
					buffer.drawImage(xps, yps, iconImageBuffer[0][itemType])
				else
					buffer.drawRectangle(xps, yps, 20, 10, 0x00, 0, " ")
				end
			end
	...
	if showItemData and invcTItem ~= 0 then
		buffer.drawRectangle(game._gui.inventory.b1.x,game._gui.inventory.b1.y,unicode.len(textRemoveItem),1,0x3c539e, 0, " ")
		buffer.drawText(game._gui.inventory.b1.x,game._gui.inventory.b1.y,0xFEFEFE,textRemoveItem)
		-- описание
		game._function.drawItemDescription(invIdx, invIdy)
	end

end

game._gui.inventory.action["touch"] = function(ev)
	local x, y, w, h = game._gui.inventory.x, game._gui.inventory.y, game._gui.inventory.w, game._gui.inventory.h
	local ax, ay = game._gui.inventory.ax, game._gui.inventory.ay

	local formula, pItem, pType
	local itemType
	if clicked(ev[3],ev[4], x + w - 8, y, x + w - 1, y) then -- Кнопка "закрыть"
		game._data.windowThread = "pause"
		game._function.cleanIconImageBuffer()
	end
	-- кнопка выбросить предмет
	if showItemData and game._gui.inventory.target ~= 0 and clicked(ev[3],ev[4],game._gui.inventory.b1.x,game._gui.inventory.b1.y,game._gui.inventory.b1.x+game._gui.inventory.b1.w,game._gui.inventory.b1.y) then
		-- чистка памяти при утрате предмета
		if game._player.inventory["bag"][game._gui.inventory.target][1] >= game._config.reservedItemId then
			gid[game._player.inventory["bag"][game._gui.inventory.target][1]] = nil
		end
		-- пустая ячейка в инв.
		game._player.inventory["bag"][game._gui.inventory.target] = {0, 0, {}}
		-- чистка ячейки буфера
		game._function.unloadImage(iconImageBuffer[game._gui.inventory.target])
		iconImageBuffer[game._gui.inventory.target] = nil
		-- скрыть описание
		showItemData, game._gui.inventory.target = false, 0
	end

	local fbParam = true
	local temp, temp2, xps, yps
	for f = 1, 4 do
		for i = 1, 5 do
			...
			if game._player.inventory["bag"][formula][1] ~= 0 and game._player.inventory["bag"][formula][2] ~= 0 then
				...
				pItem = gid[game._player.inventory["bag"][formula][1]]
					if ev[5] == 0 then
						invcTItem = game._player.inventory["bag"][formula][1]
						game._gui.inventory.target = formula
						invTItem = game._player.inventory["bag"][formula][2]
						game._function.getItemInfo(game._player.inventory["bag"][formula][1])
						showItemData = true
						invIdx, invIdy = ev[3], ev[4]
						fbParam = false
						break
					elseif ev[5] == 1 and gid[game._player.inventory["bag"][formula][1]] then
				-- Броня
						if pItem["type"] == 2 and game._function.getItemRequirement(game._player.inventory["bag"][formula][1]) then
							pType = game._function.getArmorType(game._player.inventory["bag"][formula][1])
							if game._player.inventory["weared"][pType][1] == 0 then -- если не надето такое же
								game._player.inventory["weared"][pType] = game._player.inventory["bag"][formula] -- норм
								iconImageBuffer[pType] = iconImageBuffer[formula]
								game._player.inventory["bag"][formula] = {0, 0, {}}
								iconImageBuffer[formula] = nil
							else -- если надето такое же
								temp = game._player.inventory["weared"][pType]
								temp2 = iconImageBuffer[pType]
								game._player.inventory["weared"][pType] = game._player.inventory["bag"][formula] -- норм
								iconImageBuffer[pType] = iconImageBuffer[formula]
								iconImageBuffer[formula] = temp2
								game._player.inventory["bag"][formula] = temp -- норм
								temp2 = nil
							end
				-- Оружие
						elseif pItem["type"] == 3 and game._function.getItemRequirement(game._player.inventory["bag"][formula][1]) then
							if game._player.inventory["weared"]["weapon"][1] == 0 then -- если не надето такое же
								game._player.inventory["weared"]["weapon"] = game._player.inventory["bag"][formula] -- норм
								iconImageBuffer["weapon"] = iconImageBuffer[formula]
								game._player.inventory["bag"][formula] = {0, 0, {}}
								iconImageBuffer[formula] = nil
							else -- если надето такое же
								temp = game._player.inventory["weared"]["weapon"]
								temp2 = iconImageBuffer["weapon"]
								game._player.inventory["weared"]["weapon"] = game._player.inventory["bag"][formula] -- норм
								iconImageBuffer["weapon"] = iconImageBuffer[formula]
								iconImageBuffer[formula] = temp2
								game._player.inventory["bag"][formula] = temp -- норм
	...
	for f = 1, 4 do
		for i = 1, 2 do
			...
			if game._player.inventory["weared"][itemType][1] ~= 0 then
				if clicked(ev[3],ev[4],xps,yps,xps+19,yps+9) then
					if ev[5] == 0 then -- описание
						invcTItem = game._player.inventory["weared"][itemType][1]
						invTItem = 1
						showItemData = true
						game._function.getItemInfo(game._player.inventory["weared"][itemType][1])
						invIdx, invIdy = ev[3], ev[4]
						fbParam = false
						break
					elseif ev[5] == 1 then -- снять
						for n = 1, #game._player.inventory["bag"] do
							if game._player.inventory["bag"][n][1] == 0 then
								game._player.inventory["bag"][n] = game._player.inventory["weared"][itemType]
								iconImageBuffer[n] = iconImageBuffer[itemType]
								game._player.inventory["weared"][itemType] = {0, 0, {}}
								iconImageBuffer[itemType] = nil
								break
							end
						end
					end
				...
				if clicked(ev[3],ev[4],xps,yps,xps+19,yps+9) then -- надпись
					if ev[5] == 0 then
						invcTItem = 1
						showItemData = true
						game._data.itemInfo = {{game._function.getWitemTypeName(itemType), 0xFFFFFF}}
						invIdx, invIdy = ev[3], ev[4]
						fbParam = false
					end
				end
...
buffer.drawText(x+1,y,0xffffff,"Монеты "..game._player.coins)
...
buffer.drawText(x+119, y+2,0xffffff,hclr[game._function.tradew.torg])
 ...
 buffer.drawText(x+65,y+3,0xC2C2C2,"Цена за единицу")
  ...
  local td = clr
  if game._function.tradew.titemcount*game._function.tradew.loaded[game._function.tradew.sect][game._function.tradew.titem]["cost"] > game._player.coins then td = 0xb71202 end
  ...
  game._function.drawItemDescription(smx+game._function.tradew.cWidth+2,smy+1)
  end
 elseif game._function.tradew.torg == 2 then
  ...
  for f = 1, #game._player.inventory["bag"] do
   if game._player.inventory["bag"][f][1] ~= 0 and game._player.inventory["bag"][f][2] ~= 0 then
   tableInsert(game._function.tradew.asmt,game._player.inventory["bag"][f])
   end
  end
  for f = 1, 25 do
  ...
  buffer.drawText(x+70,y+4+f,0xDDDDDD,gid[game._function.tradew.asmt[f][1]]["cost"]..game._function.genitiveWordEnding(" монет",gid[game._function.tradew.asmt[f][1]]["cost"]))
   ...
   game._function.drawItemDescription(91,20)
...
game._function.tradew.action["touch"] = function(ev)
	...
	if game._function.tradew.torg == 2 then -- Продажа
		for f = 1, #game._function.tradew.asmt do
			if ev[5] == 0 and clicked(ev[3],ev[4],2,5+f,85,5+f) then
				...
				game._function.getItemInfo(game._function.tradew.asmt[f][1])
		...
		if game._function.tradew.titem > 0 then
			if ev[5] == 0 then
				...
				if clicked(ev[3],ev[4],130,7,145,9) then
					game._function.removeItem(game._function.tradew.asmt[game._function.tradew.titem][1], game._function.tradew.titemcount) -- Тест
					game._function.addCoins(gid[game._function.tradew.asmt[game._function.tradew.titem][1]]["cost"] * game._function.tradew.titemcount)
					game._function.cleanIconImageBuffer()
					game._function.tradew.titem = 0
					game._function.tradew.titemcount = 1
				end
	...
	elseif game._function.tradew.torg == 1 and game._function.tradew.titem == 0 then -- Покупка
		...
		for c = 1, mathMin(#game._function.tradew.loaded[game._function.tradew.sect], 24) do
			if clicked(ev[3],ev[4],2,5+c*2-2,160,5+c*2) then
				...
				game._function.getItemInfo(game._function.tradew.loaded[game._function.tradew.sect][game._function.tradew.titem]["item"])
	...
	elseif game._function.tradew.torg == 1 and game._function.tradew.titem > 0 then
		if ev[5] == 0 and gid[game._function.tradew.loaded[game._function.tradew.sect][game._function.tradew.titem]["item"]]["stack"] and game._function.tradew.titemcount < 100 and clicked(ev[3],ev[4],game._function.tradew.twx+13, game._function.tradew.twy+3,game._function.tradew.twx+13, game._function.tradew.twy+3) then -- +
			game._function.tradew.titemcount = game._function.tradew.titemcount + 1
		...
		-- купить
		if clicked(ev[3],ev[4],game._function.tradew.twx,game._function.tradew.twy+game._function.tradew.cHeight,game._function.tradew.twx+game._function.tradew.cWidth,game._function.tradew.twy+game._function.tradew.cHeight+3) and game._player.coins >= game._function.tradew.titemcount*game._function.tradew.loaded[game._function.tradew.sect][game._function.tradew.titem]["cost"] then
			game._player.coins = game._player.coins - game._function.tradew.titemcount*game._function.tradew.loaded[game._function.tradew.sect][game._function.tradew.titem]["cost"]
...
buffer.drawText(x+1,y+2,0xffffff,"Монеты "..game._player.coins)
 --------------
	local t1, n1, hclr
	for i = 1, 2 do
		for f = 1, 5 do
 ...
 local td
 if game._function.craftw.titemcount*game._function.craftw.loaded[game._function.craftw.sect][game._function.craftw.titem]["cost"] <= game._player.coins then td = clr else td = 0xb71202 end
 ...
 game._function.drawItemDescription(smx+game._function.craftw.cWidth+2,smy+1)
 end
end
 ...
 game._data.itemInfo = nil
   ...
   if game._function.craftw.titem == 0 then
	...
	for c = 1, mathMin(#game._function.craftw.loaded[game._function.craftw.sect], 24) do
	 ...
	 game._function.getItemInfo(game._function.craftw.loaded[game._function.craftw.sect][game._function.craftw.titem]["item"])
    ...
    if ev[5] == 0 and gid[game._function.craftw.loaded[game._function.craftw.sect][game._function.craftw.titem]["item"]]["stack"] and game._function.craftw.titemcount < 100 and clicked(ev[3],ev[4],game._function.craftw.twx+13, game._function.craftw.twy+3,game._function.craftw.twx+13, game._function.craftw.twy+3) then
	...
	if game._function.craftw.loaded[game._function.craftw.sect][game._function.craftw.titem]["cost"] > game._player.coins then checkVar1 = false end
	 if checkVar1 then
	  for d = 1, #game._function.craftw.loaded[game._function.craftw.sect][game._function.craftw.titem]["recipe"] do
	   for i = 1, #game._player.inventory["bag"] do
	    if game._player.inventory["bag"][i][1] == game._function.craftw.loaded[game._function.craftw.sect][game._function.craftw.titem]["recipe"][d][1] then
	    game._player.inventory["bag"][i][2] = game._player.inventory["bag"][i][2] - game._function.craftw.loaded[game._function.craftw.sect][game._function.craftw.titem]["recipe"][d][2]*game._function.craftw.titemcount
	    if game._player.inventory["bag"][i][2] == 0 then game._player.inventory["bag"][i][1] = 0 end
		break
		end
	   end
	  end
	 for d = 1, game._function.craftw.titemcount do
	  ...
	  game._player.coins = game._player.coins - game._function.craftw.loaded[game._function.craftw.sect][game._function.craftw.titem]["cost"]
		...
		f=function()
			...
			for f=1,#game._player.inventory["bag"] do
				if game._function.random(0,100) <= 1 then
					game._player.inventory["bag"][f][2] = game._player.inventory["bag"][f][2] - 1
				end
			end
			CGD[game._player.id]["targ"] = nil
...
function game._gui.playerStats.updateContent()
	game._gui.playerStats.info = {
		...
		"Вероятность нанесения критического удара: "..game._player.critChance.."%",
	}
end
...
game._gui.playerStats.action["touch"] = function(ev)
	...
	for t = 1, 4 do
		if ev[5] == 0 and clicked(ev[3], ev[4], x1 + 17, y1 + t, x1 + 20, y1 + t) and CGD[game._player.id]["levelpoints"] > 0  then
		...
		elseif ev[5] == 0 and clicked(ev[3],ev[4],x1 + 22, y1 + t, x1 + 25, y1 + t) and game._gui.playerStats.selectedPoints[t] > 0 then
			game._gui.playerStats.selectedPoints[t] = game._gui.playerStats.selectedPoints[t] - 1
			CGD[game._player.id]["levelpoints"] = CGD[game._player.id]["levelpoints"] + 1
			game._gui.playerStats.selectedPoints[5] = game._gui.playerStats.selectedPoints[5] - 1
		end
...
function game._gui.playerSkills.draw()
	...
	for f = 1, #game._player.skills do
		...
		cnm = gsd[game._player.skills[f][1]]["name"].." ("..game._player.skills[f][3].." ур.)"
		buffer.drawText(x+mathFloor(25-unicode.len(cnm)/2),y+3+f*3-3,0xffffff,cnm)
	end
	
	if game._gui.playerSkills.targ ~= 0 then
		
		local slvl = mathMax(game._player.skills[game._gui.playerSkills.targ][3], 0)
		targetSkill = gsd[game._player.skills[game._gui.playerSkills.targ][1]]
		if (targetSkill["type"] == 2 and slvl < game._config.maxSkillLevel) or (targetSkill["type"] ~= 2 and slvl < #targetSkill["manacost"]) then
			buffer.drawRectangle(x+55, y+30, 46, 8, 0xA3A3A3, 0, " ")

			game._function.pSkillsPbar(x + 55, y + 25, slvl)

			for f = 1, #game._gui.playerSkills.text3 do -- Текст
				buffer.drawText(x+57,y+30+f,game._gui.playerSkills.text3[f][2],tostring(game._gui.playerSkills.text3[f][1]))
			end
		...
		for i = 1, #game._gui.playerSkills.text1 do -- Текст
			buffer.drawText(x + 54,y + 2 + i, 0xffffff, game._gui.playerSkills.text1[i])
		end
		
		for f = 1, #game._gui.playerSkills.text2 do -- Текст
			buffer.drawText(x+54,y+9+f,0xffffff,tostring(game._gui.playerSkills.text2[f]))
		end
		...
		for p = 1, #game._gui.skillsTopPanel.t do
			...
			for n = 1, #game._player.actSkills do
				if game._player.actSkills[p + 1] == game._gui.playerSkills.targ then
					bColor = 0xBBBBBB
					break
				end
			end
	...
	if game._gui.playerSkills.targ > 0 then
		...
		if ev[5] == 0 and clicked(ev[3],ev[4],x+70,game._gui.playerSkills.window.y+35,x+84,y+37) and ((targetSkill["type"] == 2 and slvl < game._config.maxSkillLevel) or (targetSkill["type"] ~= 2 and slvl < #targetSkill["manacost"])) then
			...
			if targetSkill["reqlvl"] then
				if targetSkill["reqlvl"][slvl + 1] > CGD[game._player.id]["lvl"] then
					checkv1 = false
				end
			end
			if targetSkill["reqcn"] then
				if targetSkill["reqcn"][slvl + 1] > game._player.coins then
					checkv1 = false
				else
					checkv2.c = true
				end
			end
			if targetSkill["reqitem"] and targetSkill["reqitem"][slvl + 1] then
				if game._function.checkItemInBag(targetSkill["reqitem"][slvl + 1][1]) < targetSkill["reqitem"][slvl + 1][2] then
					checkv1 = false
				else
					checkv2.o, checkv2.i = game._function.checkItemInBag(targetSkill["reqitem"][slvl + 1][1])
				end
			end
			if checkv1 == true then
				if checkv2.c then
					game._player.coins = game._player.coins - targetSkill["reqcn"][slvl + 1]
				end
				if checkv2.i then
					game._function.removeItem(targetSkill["reqitem"][slvl + 1][1], targetSkill["reqitem"][slvl + 1][2]) -- Забрать предмет из инв.
				end
				game._player.skills[game._gui.playerSkills.targ][3] = slvl + 1
				game._gui.playerSkills.updateDescription()
			end
		end
		for p = 1, #game._player.actSkills do
			if ev[5] == 0 and clicked(ev[3],ev[4],x+105,y+6+4*p-4,x+115,y+6+4*p-1) and game._player.skills[game._gui.playerSkills.targ][1] > 1 and slvl > 0 and gsd[game._player.skills[game._gui.playerSkills.targ][1]]["type"] ~= 2 then
				for n = 1, #game._player.actSkills do
					if game._player.actSkills[n] == game._gui.playerSkills.targ then
...
function game._gui.playerSkills.updateDescription()
	local targetSkill, slvl
	if game._gui.playerSkills.targ ~= 0 then
		targetSkill = gsd[game._player.skills[game._gui.playerSkills.targ][1]]
		game._gui.playerSkills.text1 = {}
		game._gui.playerSkills.text2 = {}
		slvl = mathMax(game._player.skills[game._gui.playerSkills.targ][3],1)

		-- тест V
		local typeDmgText = {[0] = "физического", [1] = "магического"}
		rv = {}
		if targetSkill["value"] then
			rv[1] = targetSkill["value"][slvl]
		else
			rv[1] = ""
		end
		if targetSkill["bseatckinc"] then
			rv[2] = targetSkill["bseatckinc"][slvl]
		else
			rv[2] = ""
		end
		if type(targetSkill["basedmgmlt"]) == "table" then
			rv[3] = targetSkill["basedmgmlt"][slvl]
		elseif type(targetSkill["basedmgmlt"]) == "number" then
			rv[3] = targetSkill["basedmgmlt"]
		else
			rv[3] = ""
		end
		if targetSkill["weapondmgmlt"] then
			rv[4] = targetSkill["weapondmgmlt"][slvl] else rv[4] = ""
		end
		if targetSkill["eff"] and ged[targetSkill["eff"]]["dur"] then
			rv[5] = ged[targetSkill["eff"]]["dur"][slvl]
		else
			rv[5] = ""
		end
		if targetSkill["eff"] and ged[targetSkill["eff"]]["val"] then
			rv[6] = math.abs(ged[targetSkill["eff"]]["val"][slvl])
		else
			rv[6] = ""
		end
		...
		slvl = game._player.skills[game._gui.playerSkills.targ][3]
		ntt = {
			["a"]=typeDmgText[targetSkill["typedm"]],
			["b"]=rv[1],
			["c"]=rv[2],
			["i"]=rv[3],
			["e"]=rv[4],
		...
		tableInsert(game._gui.playerSkills.text1, "•" .. targetSkill["name"])
		tableInsert(game._gui.playerSkills.text1, "Тип: " .. game._data.skillType[targetSkill["type"]])
		if slvl > 0 and (targetSkill["type"] ~= 2) then
			tableInsert(game._gui.playerSkills.text1, "Уровень умения: " .. slvl .. " / " .. #targetSkill["manacost"])
			tableInsert(game._gui.playerSkills.text1, "Использует маны: " .. targetSkill["manacost"][slvl].." ед.")
			tableInsert(game._gui.playerSkills.text1, "Перезарядка: " .. targetSkill["reloading"].." сек.")
			if targetSkill["type"] == 1 then
				tableInsert(game._gui.playerSkills.text1, "Дальность: " .. (targetSkill["distance"]+(vAttackDistance or 8)))
			end
		elseif slvl > 0 and targetSkill["type"] == 2 then
			tableInsert(game._gui.playerSkills.text1, "Уровень умения: "..slvl.." / "..#targetSkill["value"])
		else
			tableInsert(game._gui.playerSkills.text1, "Умение ещё не изучено")
		end
		...
		for m = 1, unicode.len(targetSkill["descr"]) do
			if unicode.sub(targetSkill["descr"],rv,rv) ~="$" then
				tempText = tempText..unicode.sub(targetSkill["descr"],rv,rv)
				rv = rv + 1
			else
				tempText = tempText..tostring(ntt[unicode.sub(targetSkill["descr"], rv + 1, rv + 1)])
				rv = rv + 2
			end
		end
		game._gui.playerSkills.text2 = game._function.textWrap(tempText, 42)
		
		if (targetSkill["type"] == 2 and slvl < game._config.maxSkillLevel) or (targetSkill["type"] ~= 2 and slvl < #targetSkill["manacost"]) then
			game._gui.playerSkills.text3 = {
				{"Улучшение умения • следующий уровень "..slvl + 1, 0xEFEFEF}
			}
			if targetSkill["reqlvl"] then
				tableInsert(game._gui.playerSkills.text3,{"Требуемый уровень: "..targetSkill["reqlvl"][slvl + 1], 0xEFEFEF})
				if targetSkill["reqlvl"][slvl + 1] > CGD[game._player.id]["lvl"] then
					game._gui.playerSkills.text3[#game._gui.playerSkills.text3][2] = 0xEE1414
				end
			end
			if targetSkill["reqcn"] then
				tableInsert(game._gui.playerSkills.text3,{"Стоимость улучшения: "..targetSkill["reqcn"][slvl + 1].." монет", 0xEFEFEF})
				if targetSkill["reqcn"][slvl + 1] > game._player.coins then
					game._gui.playerSkills.text3[#game._gui.playerSkills.text3][2] = 0xEE1414
				end
			end
			if targetSkill["reqitem"] and targetSkill["reqitem"][slvl + 1] then
				reqItem = targetSkill["reqitem"][slvl + 1]
				tableInsert(game._gui.playerSkills.text3,{"Требуемый предмет: "..gid[reqItem[1]]["name"].."("..game._function.checkItemInBag(reqItem[1]).."/"..reqItem[2]..")", 0xEFEFEF})
				if game._function.checkItemInBag(reqItem[1]) < reqItem[2] then
					game._gui.playerSkills.text3[#game._gui.playerSkills.text3][2] = 0xEE1414
				end
			end
		end
	end -- game._gui.playerSkills.targ
end
...
local function hitDamageText(unit, damage, crit)
	if crit then
...
function game._function.healUnit(id, value)
	if CGD[id]["chp"] + value < CGD[id]["mhp"] then
		CGD[id]["chp"] = CGD[id]["chp"] + value
	else
		CGD[id]["chp"] = CGD[id]["mhp"]
...
-- Один юнит наносит урон другому

function game._function.makeDamage(fromid, id, damage, ignoreTarget, isCritical)
	local xpGain
	if CGD[fromid] and CGD[id] and damage then
		damage = game._function.roundUpNumber(damage)
		hitDamageText(id, damage, isCritical or false)
		-- не упал
		if CGD[id]["chp"] > damage then
			CGD[id]["chp"] = CGD[id]["chp"] - damage
			if id == game._player.id then
				CGD[id]["rage"] = 10
				-- Атака прерывает выкапывание чего-то там
				if game._player.pickingUp then
					if CGD[id]["target"] == game._player.pckTarget then
						CGD[id]["target"] = nil
					end
					stopPicking()
				end
				
				if CGD[id]["target"] == nil and ignoreTarget ~= true then
					CGD[id]["target"] = fromid
				end
			else
		...
		elseif CGD[id]["chp"] <= damage then
			...
			if id == game._player.id then -- Добили игрока
				
			else -- Добили моба
				game._function.getLootItems(id) -- Выпадение лута
				game._function.huntingQuestProcessing(id) -- Задание
				xpGain = game._function.getLootExperience(id)
				addXP(xpGain) -- Опыт
				
				if CGD[fromid]["followers"] then -- Опыт союзникам
					for f = 1, #CGD[fromid]["followers"] do
						addUnitXP(CGD[fromid]["followers"][f][2], mathFloor(xpGain / #CGD[fromid]["followers"]))
					end
				end
				
				if CGD[fromid]["summoner"] and CGD[fromid]["summoner"] == game._player.id then
					addUnitXP(fromid, xpGain)
				end

				local coinsLoot = gud[CGD[id]["id"]]["loot"]["coins"] -- Деньги
				...
				if CGD[fromid]["target"] == id then
					CGD[fromid]["target"] = nil
				end
		...
		game._function.logtxt("game._function.makeDamage: ошибка данных")
	end
end
...
game._function.effect = {
	[1] = function(target, direction)
		...
		addParticles(x, y, mx, my, parts, lifeTime, 0.05)
	end,
	[2] = function(target)
		local x, y = CGD[target]["x"], 10
		for i = 1, 10 do
			addParticle(x, y, math.random(-10, 10) / 10, 0, 2, 0xff8080, 0.05)
		end
	end,
	[3] = function(target) -- Взрыв маг.
		local x, y = CGD[target]["x"] + CGD[target]["width"] / 2, 5
		local color
		for i = 1, 12 do
			...
			addParticle(x, y, math.random(-10, 10) / 10, math.random(0, 10) / 10, 1, color, 0)
		end
	end,
	[4] = function(target) -- эффект "Воспаламенение"
		local x, y = CGD[target]["x"] + CGD[target]["width"] / 2, 10
		local color
		for i = 1, 10 do
			if math.random(0, 1) == 0 then
				color = 0xffff00
			...
			addParticle(x, y, math.random(-5, 5) / 10, math.random(0, 5) / 10, 2, color, -0.01)
		end
	end,
	[5] = function(target) -- эффект "Заморозка"
		local x, y = CGD[target]["x"] + CGD[target]["width"] / 2, 10
		local color
		for i = 1, 10 do
			addParticle(x, y, math.random(-5, 5) / 10, math.random(0, 5) / 10, 2, 0x0080ff, -0.01)
		end
	end,
	[6] = function(target) -- эффект "Порез"
		local x, y = CGD[target]["x"] + CGD[target]["width"] / 2, 10
		for i = 1, 10 do
			addParticle(x, y, math.random(-5, 5) / 10, 0, 2, 0x800040, 0.01)
		end
	end,
	[7] = function(target) -- дождь
		local x, y = CGD[target]["x"] + CGD[target]["width"] / 2, 20
		for i = 1, 16 do
			addParticle(x + math.random(-10, 10) / 10, y + math.random(-20, 20) / 10, 0, math.random(0, 5) / 10, 2, 0xaaaaff, 0.01)
		end
	end,
}
...
function game._function.enemySkill(id, target, sl, lvl)
	...
	if CGD[target]["rtype"] == 1 or CGD[target]["rtype"] == 4 then
		isAvailable = true
	end
	...
	if CGD[id]["rtype"] == 1 and CGD[id]["summoner"] and CGD[id]["summoner"] == target then
		isAvailable = false
	end
	if isAvailable and eusd[sl]["type"] == 1 and CGD[id]["ctck"] then
		...
		-- Удар
			...
			game._function.makeDamage(id, target, damage) -- Урон по цели
			
			if eusd[sl]["visual"] then
				game._function.effect[eusd[sl]["visual"]](target) -- тест
			else
	...
	elseif eusd[sl]["type"] == 2 then
		...
		if eusd[sl]["visual"] then
			game._function.effect[eusd[sl]["visual"]](id) -- тест
		end
	end
end
...
function game._function.useSkill(skill)
	local cskill = game._player.skills[game._player.actSkills[skill]][1]
	local lvl = game._player.skills[game._player.actSkills[skill]][3]
	local sTarget = CGD[game._player.id]["target"] or 0
	local available, used = false, false
	
	-- Проверка, можно ли использовать
	if game._player.inventory["weared"]["weapon"][1] ~= 0 then
		if gsd[cskill]["weaponreq"] then
			for f = 1, #gsd[cskill]["weaponreq"] do
				if gid[game._player.inventory["weared"]["weapon"][1]]["subtype"] == gsd[cskill]["weaponreq"][f] then
					available = true
					break
				end
			end
	...
	if CGD[game._player.id]["targ"] then
		sTarget = CGD[game._player.id]["targ"][2]
	end

	local isQuiet = function(i) -- Заменить !!
		local types = {1, 2, 5, 3}
		for f = 1, #types do
			if i == types[f] then
				return true
	...
	local calcDamage = function(unit)
		...
		if type(gsd[cskill]["basedmgmlt"]) == "table" then -- Базовый урон от умения
		...
		if gsd[cskill]["weapondmgmlt"] and game._player.inventory["weared"]["weapon"][1] > 0 then -- Урон от оружия
			damage = damage + weaponDmg*gsd[cskill]["weapondmgmlt"][lvl]*0.01
		end
		if gsd[cskill]["bseatckinc"] then
			damage = damage + damage*gsd[cskill]["bseatckinc"][lvl] * 0.01 -- Увеличение урона на % (не исп.)
		end
		if gsd[cskill]["value"] then -- Увеличение урона на value ед.
	...
	if CGD[game._player.id]["ctck"] == true and
			CGD[game._player.id]["cmp"] >= gsd[cskill]["manacost"][lvl] and 
			game._player.skills[game._player.actSkills[skill]][2] == 0 then
	...
	if sTarget ~= 0 then
		if isQuiet(CGD[sTarget]["rtype"]) and gsd[cskill]["type"] ~= 4 then
			available = false
		end
	end

	local dmg, crit = false

	-- Если доступно
	if available then
		if gsd[cskill]["type"] == 1 then
			if sTarget ~= 0 and CGD[sTarget]["living"] and game._function.getDistanceToId(game._player.id,sTarget) <= vAttackDistance + gsd[cskill]["distance"] then
				if CGD[sTarget]["x"] > CGD[game._player.id]["x"] then -- Поворот
					setSpritePosition(game._player.id, 2)
				else
					setSpritePosition(game._player.id, 1)
				end
				if gsd[cskill]["action"] and gsd[cskill]["action"]["type"] == 1 then -- Урон по площади
					for f = 1, #CGD do
						if CGD[f] and CGD[f]["living"] and game._function.getDistanceToId(sTarget, f) <= gsd[cskill]["action"]["dist"] and not isQuiet(CGD[f]["rtype"]) then
							dmg = calcDamage(f)
							if game._function.random(1, 100) <= game._player.critChance then
								dmg = dmg * 2
								crit = true
							end
							game._function.makeDamage(game._player.id, f, dmg, false, crit) -- Урон
							if gsd[cskill]["visual"] then -- тест
								game._function.effect[gsd[cskill]["visual"]](f, CGD[game._player.id]["spos"]) -- тест
							end
							if gsd[cskill]["eff"] ~= nil then -- Эффект
								game._function.addUnitEffect(f,gsd[cskill]["eff"],game._player.skills[game._player.actSkills[skill]][3])
							end
						end
					end
				else -- Урон по одной цели
					dmg = calcDamage(sTarget)
					if game._function.random(1, 100) <= game._player.critChance then
						dmg = dmg * 2
						crit = true
					end
					game._function.makeDamage(game._player.id, sTarget, dmg, false, crit)
					if gsd[cskill]["visual"] then -- тест
						game._function.effect[gsd[cskill]["visual"]](sTarget, CGD[game._player.id]["spos"]) -- тест
					end
					if sTarget ~= 0 and gsd[cskill]["eff"] ~= nil then
						game._function.addUnitEffect(sTarget,gsd[cskill]["eff"],game._player.skills[game._player.actSkills[skill]][3])
					end
				end
		...
		elseif gsd[cskill]["type"] == 3 then -- Бафф
			if gsd[cskill]["eff"] ~= nil then
				game._function.addUnitEffect(game._player.id,gsd[cskill]["eff"],lvl)
			end
			skillUsingMsg[1] = gsd[cskill]["name"]
			used = true
		elseif gsd[cskill]["type"] == 4 then -- Восстановление хп союзника в процентах (value)
			for f = 1, #CGD[game._player.id]["followers"] do
				if sTarget == CGD[game._player.id]["followers"][f][2] then
					if CGD[sTarget]["x"] > CGD[game._player.id]["x"] then -- Поворот
						setSpritePosition(game._player.id, 2)
					else
						setSpritePosition(game._player.id, 1)
					end
					game._function.healUnit(sTarget, CGD[sTarget]["mhp"] / 100 * gsd[cskill]["value"][lvl])
					if gsd[cskill]["visual"] then -- тест
						game._function.effect[gsd[cskill]["visual"]](game._player.id) -- тест
						game._function.effect[gsd[cskill]["visual"]](sTarget)
					end
					used = true
					break
				end
		...
		if used then
			...
			game._player.skills[game._player.actSkills[skill]][2] = gsd[cskill]["reloading"] * 10
			setPlayerImage(-4)
			pimg4t = 0
			vtskillUsingMsg = 3
			skillUsingMsg[1] = gsd[cskill]["name"]
...
function game._function.usePlayerSkill(skill)
	local skillType, unit
	if game._player.actSkills[skill] > 0 and game._player.skills[game._player.actSkills[skill]] and game._player.skills[game._player.actSkills[skill]][3] > 0 and not game._player.pickingUp then
		skillType = gsd[game._player.skills[game._player.actSkills[skill]][1]]["type"]
		if skillType == 1 or skillType == 4 then
			if CGD[game._player.id]["target"] ~= nil then
				vAttackDistance = vAttackDistance or 8
				if game._function.getDistanceToId(game._player.id, CGD[game._player.id]["target"]) > game._function.getPlayerAtdsBySkill(skill) then
					CGD[game._player.id]["targ"] = {2, CGD[game._player.id]["target"], game._function.getPlayerAtdsBySkill(skill), skill}
				else
					game._function.useSkill(skill)
					CGD[game._player.id]["targ"] = {2, CGD[game._player.id]["target"], game._function.getPlayerAtdsBySkill(skill), skill}
					game._player.moving = false
					pmov = 0
...
function game._function.pickUpResource(id)
	...
	if CGD[id] then
		if CGD[id]["reqquest"] then
			available = false
			for f = 1, #game._player.quests do
				if game._player.quests[f][1] == CGD[id]["reqquest"] then
		...
		if available then
			...
			local mpcktime = game._function.random(gud[CGD[id]["id"]]["mnprs"] * 10, gud[CGD[id]["id"]]["mxprs"] * 10)
...
function game._function.getQuestReward(id)
	if gqd[id] then
		...
		game._player.coins = game._player.coins + gqd[id]["qreward"]["coins"]
...
function game._function.gathering()
	if not game._player.pickingUp then
		if game._function.getDistanceToId(game._player.id,CGD[game._player.id]["target"]) <= 11 then
			...
			CGD[game._player.id]["targ"] = {1,CGD[game._player.id]["target"],5}
			game._function.getClose(game._player.id, CGD[game._player.id]["target"], 5)
			game._player.moving = true
		end
	end
end

function game._function.getClose(id, cid, distance)
	if CGD[id]["x"] > mathFloor(CGD[cid]["x"]) + CGD[cid]["width"] then
		CGD[id]["mx"] = mathFloor(CGD[cid]["x"]) + CGD[cid]["width"] + game._function.roundUpNumber(distance)
	elseif CGD[id]["x"] + CGD[id]["width"] < mathFloor(CGD[cid]["x"]) then
		CGD[id]["mx"] = mathCeil(CGD[cid]["x"]) - CGD[id]["width"] - game._function.roundUpNumber(distance)
	end
end
...
function game._function.debugText()
	local text = {
		"#savedUnits = "..tostring(countSavedUnits()),
		"#CGD = "..#CGD,
		"#gid = "..#gid,
...
function game._gui.qPanel.update()
...
local function drawPlayer()
	if CGD[game._player.id]["spos"] == 2 then
		buffer.drawImage(game._player.screenPosition, 49 - CGD[game._player.id]["height"] - game._function.roundUpNumber(CGD[game._player.id]["y"]), imageBuffer[CGD[game._player.id]["image"]])
	else
		buffer.drawImage(game._player.screenPosition, 49 - CGD[game._player.id]["height"] - game._function.roundUpNumber(CGD[game._player.id]["y"]), image.flipHorizontally(imageBuffer[CGD[game._player.id]["image"]]))
	end
...
local function dmain()
	...
	elseif game._data.windowThread == "inventory" then  game._gui.inventory.draw()
	...
	elseif game._data.windowThread == "died" then game._function.ydw.draw()
	...
	buffer.drawText(1,mxh,fpstclr,tostring(cfps)) -- fps
	usram = game._function.RAMInfo()
	buffer.drawText(mxw-#usram,50,0xC7C7C7,usram)
...
function game._function.initGame()
	...
	game._player.inventory = {
		["weared"] = {
			...
			["robe"] = {0, 0, {}},
			["pants"] = {0, 0, {}},
			["weapon"] = {game._config.startWeapon[game._player.class], 1, {}},
			["footwear"] = {0, 0, {}},
			["ring"] = {0, 0, {}}
		},
		["bag"] = {}
	...
	for f = 1, game._parameter.inventorySize do
		game._player.inventory["bag"][f] = {0, 0, {}}
	end
	-- Данные
	gud[CGD[game._player.id]["id"]]["name"] = game._player.name
	CGD[game._player.id]["levelpoints"] = gud[game._player.gudId]["lvl"] * 2 - 2
	CGD[game._player.id]["surv"] = gud[game._player.gudId]["surv"]
	CGD[game._player.id]["strg"] = gud[game._player.gudId]["strg"]
	CGD[game._player.id]["int"] = gud[game._player.gudId]["int"] 
	CGD[game._player.id]["agi"] = gud[game._player.gudId]["agi"]
	CGD[game._player.id]["followers"] = {}
	CGD[game._player.id]["target"] = nil
	...
	game._player.coins = 0
	CGD[game._player.id]["targ"] = nil
	CGD[game._player.id]["vx"] = 0
	CGD[game._player.id]["vy"] = 0
	CGD[game._player.id]["spos"] = 2
	CGD[game._player.id]["petcage"] = {}

	--addXP(5000000)-- !!!

	-- Умения (соответствующие классу)
	game._player.skills = {}
	for f = 1, #playerParams[game._player.class]["skills"] do
		game._player.skills[f] = {}
		for n = 1, #playerParams[game._player.class]["skills"][f] do
			game._player.skills[f][n] = playerParams[game._player.class]["skills"][f][n]
		end
	end

	game._function.loadWorld(world.current) -- Загрузить мир
	game._function.UpdatePlayerStats() -- Определить характеристики
	game._function.maxXP()
	CGD[game._player.id]["chp"] = CGD[game._player.id]["mhp"]
	CGD[game._player.id]["cmp"] = CGD[game._player.id]["mmp"]
...
function game._gui.mainMenu.draw()
	...
	for f = 1, #game._gui.mainMenu.obj do
		buffer.drawText(game._function.roundUpNumber(game._gui.mainMenu.obj[f].x),mxh-game._function.roundUpNumber(game._gui.mainMenu.obj[f].y),0x575757,"*")
	end
	...
	if game._gui.mainMenu.mode == 0 then
		for f = 1, #mainMenulist do
			if ev[5] == 0 and clicked(ev[3],ev[4],game._gui.mainMenu.x,game._gui.mainMenu.y+f*4-3,game._gui.mainMenu.x+game._gui.mainMenu.w-1,game._gui.mainMenu.y+f*4+2) then
				stat, err = pcall(game._gui.mainMenu.MenuAction[f])
				ev[3], ev[4] = 0, 0
				break
			end
		end
	elseif game._gui.mainMenu.mode == 1 then
		for f = 1, #game._gui.mainMenu[1].obj do
			if ev[5] == 0 and game._gui.mainMenu[1].obj[f].type == 1 and clicked(ev[3],ev[4],game._gui.mainMenu[1].x+game._gui.mainMenu[1].obj[f].x,game._gui.mainMenu[1].y+game._gui.mainMenu[1].obj[f].y,game._gui.mainMenu[1].x+game._gui.mainMenu[1].obj[f].x+game._gui.mainMenu[1].obj[f].w-1,game._gui.mainMenu[1].y+game._gui.mainMenu[1].obj[f].y+game._gui.mainMenu[1].obj[f].h-1) then
				stat, err = pcall(game._gui.mainMenu[1].obj[f].action)
			end
		end
	elseif game._gui.mainMenu.mode == 2 then
		for f = 1, #game._gui.mainMenu[2].obj do
			if ev[5] == 0 and game._gui.mainMenu[2].obj[f].type == 1 and clicked(ev[3],ev[4],game._gui.mainMenu[2].x+game._gui.mainMenu[2].obj[f].x,game._gui.mainMenu[2].y+game._gui.mainMenu[2].obj[f].y,game._gui.mainMenu[2].x+game._gui.mainMenu[2].obj[f].x+game._gui.mainMenu[2].obj[f].w-1,game._gui.mainMenu[2].y+game._gui.mainMenu[2].obj[f].y+game._gui.mainMenu[2].obj[f].h-1) then
				stat, err = pcall(game._gui.mainMenu[2].obj[f].action)
			end
		end
		for f = 1, #game._gui.mainMenu[2].buff do
...
local function functionPS()
	local value, duration, effSourceId, effObj, efftype, itemLootarray, qwert, asdf, cl
	local uMoveRef = 8
	local deltan = 0
	while game._data.inGame do
		...
		if not game._data.paused then
			...
			if CGD[game._player.id]["rage"] > 0 then
				game._player.regenMultiplier = 0.1
				CGD[game._player.id]["rage"] = CGD[game._player.id]["rage"] - 1
			else
				game._player.regenMultiplier = 1
			end
			...
			if CGD[game._player.id]["living"] then
				...
				if CGD[game._player.id]["cmp"] < CGD[game._player.id]["mmp"] - game._player.manaReg then
					CGD[game._player.id]["cmp"] = CGD[game._player.id]["cmp"] + game._player.manaReg
				else
					CGD[game._player.id]["cmp"] = CGD[game._player.id]["mmp"]
				end
				-- восстановление здоровья персонажа
				game._function.addHealth(game._player.id, game._player.healthReg)
			end
			-- респавн во всех измерениях
			for f = 1, #savedUnits do
				if f ~= world.current and savedUnits[f] then
					for n = #savedUnits[f], 1, -1 do
						if savedUnits[f][n][4] > 0 then
							savedUnits[f][n][4] = savedUnits[f][n][4] - 1
						else
							tableRemove(savedUnits[f], n)
						end
					end
				end
			end
				...
				for f = 1, #CGD[game._player.id]["followers"] do
					if not CGD[CGD[game._player.id]["followers"][f][2]]["target"] and game._function.getDistanceToId(CGD[game._player.id]["followers"][f][2], game._player.id) > 10 then
						if game._function.getDistanceToId(CGD[game._player.id]["followers"][f][2], game._player.id) > 160 then
							...
							game._function.getClose(CGD[game._player.id]["followers"][f][2], game._player.id, 8)
						end
					end
			...
			for f = 1, #CGD do
				if CGD[f] then
					if CGD[f]["living"] then
						...
						if uMoveRef == 0 then
							if CGD[f]["rtype"] == 4 and game._function.random(1, 4) == 4 then
								CGD[f]["mx"] = CGD[f]["sx"] + game._function.random(-8, 8)
							end
						end

						-- моб подходит и бьёт игрока
						if (CGD[f]["rtype"] == 4 or CGD[f]["rtype"] == 1) and CGD[f]["target"] and f ~= game._player.id then
					...
					-- агр мобов
						if CGD[f]["rtype"] == 4 and CGD[f]["living"] and gud[CGD[f]["id"]]["agr"] == 1 then
							for n = 1, #CGD do
								if CGD[n]["rtype"] == 1 and CGD[n]["living"] and game._function.getDistanceToId(n,f) <= 25 then
									CGD[f]["target"] = n
									break
								end
							end
					...
					for eff = #CGD[f]["effects"], 1, -1 do
						effObj = CGD[f]["effects"][eff]
						if CGD[f]["living"] and effObj ~= nil then
							if ged[effObj[1]]["visual"] then
								game._function.effect[ged[effObj[1]]["visual"]](f) -- тест
							end
						
							if ged[effObj[1]]["val"] then
								value = ged[effObj[1]]["val"][effObj[3]]
							end
							effSourceId = effObj[4] or f -- Источник (кто налождил эффект) - если нет, то = сам юнит
							duration = ged[effObj[1]]["dur"][effObj[3]] -- Продолжительность
							efftype = ged[effObj[1]]["type"] -- Тип
							if efftype == "hpi" then -- Увеличение хп на 'value' за 'duration' секунд
								game._function.healUnit(f, value / duration)
							...
							elseif efftype == "hpd" then -- Уменьшение хп на 'value' за 'duration' секунд
								game._function.makeDamage(effSourceId, f, value / duration, true)
							...
							if effObj ~= nil then
								effObj[2] = effObj[2] - 1
								if effObj[2] == 0 then -- Действие эффекта закончилось
									if game._gui.playerInfoPanel.effectDescription == effObj[1] then
										game._gui.playerInfoPanel.effectDescription = 0 -- Если было открыто описание, закрыть
									end
									tableRemove(CGD[f]["effects"], eff)
								end
							end
						end
					end
		...
		-- Выдает предмет который не поместился в инвентарь
			if lostItem and not game._function.checkInventoryisFull() then 
				game._function.addItem(lostItem[1],lostItem[2],true)
				lostItem = nil
			end
		-- Амулет, автовосстановление жс, мэ
			if (CGD[game._player.id]["chp"] < CGD[game._player.id]["mhp"] / 2) or (CGD[game._player.id]["cmp"] < CGD[game._player.id]["mmp"] / 2) then
				for f = 1, #game._player.inventory["bag"] do
					if game._player.inventory["bag"][f][1] > 0 and game._player.inventory["bag"][f][2] > 0 and gid[game._player.inventory["bag"][f][1]] and CGD[game._player.id]["living"] and gid[game._player.inventory["bag"][f][1]]["type"] == 7 then
						if gid[game._player.inventory["bag"][f][1]]["subtype"] == 1 and CGD[game._player.id]["chp"] <= CGD[game._player.id]["mhp"]*(gid[game._player.inventory["bag"][f][1]]["props"]["r"]*0.01) then
							CGD[game._player.id]["chp"] = mathMin(CGD[game._player.id]["chp"]+CGD[game._player.id]["mhp"]*0.01*gid[game._player.inventory["bag"][f][1]]["props"]["ics"],CGD[game._player.id]["mhp"])
							game._player.inventory["bag"][f][2] = game._player.inventory["bag"][f][2] - 1
							break
						elseif gid[game._player.inventory["bag"][f][1]]["subtype"] == 2 and CGD[game._player.id]["cmp"] <= CGD[game._player.id]["mmp"]*(gid[game._player.inventory["bag"][f][1]]["props"]["r"]*0.01) then
							CGD[game._player.id]["cmp"] = mathMin(CGD[game._player.id]["cmp"]+CGD[game._player.id]["mmp"]*0.01*gid[game._player.inventory["bag"][f][1]]["props"]["ics"],CGD[game._player.id]["mmp"])
							game._player.inventory["bag"][f][2] = game._player.inventory["bag"][f][2] - 1
							break
						end
					end
	...
	-- while game._data.inGame do
		-- if not game._data.paused then
			-- for f = 1, #CGD do
				-- if CGD[f] then
				-- -- мобы бегают быстрее
					-- if CGD[f]["rtype"] == 4 and CGD[f]["living"] and CGD[f]["x"] ~= CGD[f]["mx"] and game._function.getDistanceToId(game._player.id,f) <= 256  and not gud[CGD[f]["id"]]["cmve"] and CGD[f]["cmove"] then
						-- tblpbl = 0.25
					...
					-- if CGD[f]["rtype"] == 1 and f ~= game._player.id and CGD[f]["living"] and game._function.getDistanceToId(game._player.id,f) <= 256 and CGD[f]["x"] ~= CGD[f]["mx"] and not gud[CGD[f]["id"]]["cmve"] and CGD[f]["cmove"] then
			...
			--deltan = os.clock()
				...
				-- умения перезаряжаются
					for f = 1, #game._player.actSkills do
						if game._player.actSkills[f] > 0 and game._player.skills[game._player.actSkills[f]][1] > 0 and game._player.skills[game._player.actSkills[f]][2] > 0 then
							game._player.skills[game._player.actSkills[f]][2] = mathMax(game._player.skills[game._player.actSkills[f]][2] - 1, 0)
						end
					end

					if CGD[game._player.id]["targ"] and CGD[game._player.id]["targ"][2] ~= 0 then
						if CGD[game._player.id]["targ"][1] == 1 and game._function.getDistanceToId(game._player.id,CGD[game._player.id]["targ"][2]) <= CGD[game._player.id]["targ"][3] then
							game._function.gatheringAction(CGD[game._player.id]["targ"][2])
							CGD[game._player.id]["targ"] = nil
						elseif CGD[game._player.id]["targ"][1] == 2 then
							if game._player.skills[game._player.actSkills[1]] and not game._player.pickingUp and CGD[CGD[game._player.id]["targ"][2]] and CGD[CGD[game._player.id]["targ"][2]]["living"] then
								if CGD[game._player.id]["targ"] and game._function.getDistanceToId(game._player.id,CGD[game._player.id]["targ"][2]) <= CGD[game._player.id]["targ"][3] then
									pmov = 0
									game._player.moving = false
									game._function.useSkill(CGD[game._player.id]["targ"][4] or 1)
									if CGD[game._player.id]["targ"] then
										CGD[game._player.id]["targ"][4] = 1
									end
								else
									game._function.getClose(game._player.id, CGD[game._player.id]["targ"][2], CGD[game._player.id]["targ"][3])
									game._player.moving = true
								end
							else
								CGD[game._player.id]["targ"] = nil
				...
				if dec > -1 then -- 1/20 сек.
					...
					if game._player.moving and CGD[game._player.id]["x"] ~= CGD[game._player.id]["mx"] then
					...
					for f = 1, #CGD do
						...
						-- мобы бегают быстрее
							if CGD[f]["rtype"] == 4 and CGD[f]["living"] and CGD[f]["x"] ~= CGD[f]["mx"] and game._function.getDistanceToId(game._player.id,f) <= 256  and not gud[CGD[f]["id"]]["cmve"] and CGD[f]["cmove"] then
								...
								game._function.moveToward(f, CGD[f]["mx"], 160, step)
							end

							if CGD[f]["rtype"] == 1 and f ~= game._player.id and CGD[f]["living"] and game._function.getDistanceToId(game._player.id,f) <= 256 and CGD[f]["x"] ~= CGD[f]["mx"] and not gud[CGD[f]["id"]]["cmve"] and CGD[f]["cmove"] then
								step = 1
								game._function.moveToward(f, CGD[f]["mx"], nil, step)
							end
						end
					end
	...
	while game._data.inGame do
		...
		if ev[1] == "key_down" then
			...
			if game._data.windowThread == nil and not game._data.paused then
				-- Движение
				if CGD[game._player.id]["cmove"] then
					if (ev[4] == game._parameter.keys.right1 or ev[4] == game._parameter.keys.right2) and CGD[game._player.id]["x"] <= world[world.current].limitR then -- вправо
						CGD[game._player.id]["targ"] = nil
						game._player.moving = false
						...
						goto continue
					elseif (ev[4] == game._parameter.keys.left1 or ev[4] == game._parameter.keys.left2) and CGD[game._player.id]["x"] >= world[world.current].limitL then -- влево
						CGD[game._player.id]["targ"] = nil
						game._player.moving = false
						if keyboard.isAltDown() then
				...
				-- Умения
				if ev[4] >= 2 and ev[4] <= 9 then
					for f = 1, 8 do
						if ev[4] == f + 1 then
							game._function.usePlayerSkill(f)
						end
					end
				end
				-- Нажатие клавиши 'E'
				if ev[4] == game._parameter.keys.interact and CGD[game._player.id]["target"] then
					-- на нпс
					if CGD[CGD[game._player.id]["target"]]["rtype"] == 2 then
						if game._function.getDistanceToId(game._player.id, CGD[game._player.id]["target"]) <= 30 then
							game._gui.NPCDialog.start(CGD[game._player.id]["target"])
						else
							game._function.getClose(game._player.id, CGD[game._player.id]["target"], 25)
						end
					-- на ресурс
					elseif CGD[CGD[game._player.id]["target"]]["rtype"] == 5 then
						game._function.gathering()
					-- на портал
					elseif CGD[CGD[game._player.id]["target"]]["rtype"] == 3 and not game._player.pickingUp and game._function.getDistanceToId(game._player.id,CGD[game._player.id]["target"]) <= 10 then
				...
				if ev[4] == game._parameter.keys.hp then
					for f = 1, #game._player.inventory["bag"] do
						if game._player.inventory["bag"][f][1] > 0 and game._player.inventory["bag"][f][2] > 0 and gid[game._player.inventory["bag"][f][1]]["type"] == 4 and gid[game._player.inventory["bag"][f][1]]["subtype"] == 1 and CGD[game._player.id]["lvl"] >= gid[game._player.inventory["bag"][f][1]]["required"]["lvl"] then
							game._function.addUnitEffect(game._player.id,game._parameter.healthPotionEffectID,gid[game._player.inventory["bag"][f][1]]["lvl"])
							game._player.inventory["bag"][f][2] = game._player.inventory["bag"][f][2] - 1
							break
						end
					end
				...
				elseif ev[4] == game._parameter.keys.mp then
					for f = 1, #game._player.inventory["bag"] do
						if game._player.inventory["bag"][f][1] > 0 and game._player.inventory["bag"][f][2] > 0 and gid[game._player.inventory["bag"][f][1]]["type"] == 4 and gid[game._player.inventory["bag"][f][1]]["subtype"] == 2 and CGD[game._player.id]["lvl"] >= gid[game._player.inventory["bag"][f][1]]["required"]["lvl"] then
							game._function.addUnitEffect(game._player.id,game._parameter.manaPotionEffectID,gid[game._player.inventory["bag"][f][1]]["lvl"])
							game._player.inventory["bag"][f][2] = game._player.inventory["bag"][f][2] - 1
							break
						end
					end
		...
		elseif ev[1] == "touch" then
			...
			if game._data.windowThread == nil then
				...
				if not game._data.paused then
					...
					if ev[5] == 0 then
						if not clicked(ev[3], ev[4], 1, 1, mxw, 8) then
							local gotTarget = false
							for f = 1, #CGD do
								if CGD[f] and f ~= game._player.id and CGD[f]["living"] and clicked(ev[3], ev[4], mathFloor(CGD[f]["x"])+75-cGlobalx, 49-mathFloor(CGD[f]["y"])-2-CGD[f]["height"], mathFloor(CGD[f]["x"])+75-cGlobalx+CGD[f]["width"], 49-mathFloor(CGD[f]["y"])) then
									if CGD[game._player.id]["target"] == f then
										if isRtype(CGD[game._player.id]["target"], 4) then
											game._function.usePlayerSkill(1) -- Атака ЛКМ, если выделено
										elseif isRtype(CGD[game._player.id]["target"], 5) then
											game._function.gathering() -- Собирание ЛКМ, если выделено
										end
									else
										CGD[game._player.id]["target"] = f -- Выделение ЛКМ
									end
									gotTarget = true
								end
							end
							if not gotTarget and CGD[game._player.id]["target"] ~= 1 and not game._gui.targetInfoPanel.showTargetInfo then
								CGD[game._player.id]["target"] = nil
							end
						end
						-- Выделить себя
						if clicked(ev[3],ev[4],game._gui.playerInfoPanel.x,game._gui.playerInfoPanel.y,game._gui.playerInfoPanel.x+game._gui.playerInfoPanel.w-1,game._gui.playerInfoPanel.y+game._gui.playerInfoPanel.h-1) then
			...
			elseif game._data.windowThread == "inventory" then
				game._gui.inventory.action["touch"](ev)
			...
			elseif game._data.windowThread == "died" then
